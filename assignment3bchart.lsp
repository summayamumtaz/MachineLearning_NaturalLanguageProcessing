...
(defun read-grammar (filename)
  ...
  ;;; log-conversion
          ...
          (let ( (grammar (make-grammar :start "start" :lexemes (make-hash-table :test 'equalp)
...
(defun parse-tree (tree grammar)
  ...
  (dolist (child (cdr tree))
    ...
    (defun parseval ( tree goldstandard)
              (if tree
  	             (let ( (goldst (traverse goldstandard))
                            	       (parsehyp (traverse tree))
                            		     (count 0))
                            	   (loop for x in goldst
                            	    do (if (position x parsehyp :test #'equal)
                            		   (incf count))
                            		)
                            	   (values count (length parsehyp) (length goldst)))

  	    ;;; if parsehyp is empty tree
  	     (let ( (goldst (traverse goldstandard))
  		    ( count 0))
         (values count  0  (length goldst)))))
                          ...
                          ;; The label bracket  (NP (NP (NN "wasabi"))  is incorrect in this as gold standard has (NP (NN "wasabi")))
 ...
 ;;;;;; question 3 part c
;;;
;;; These unknow words will not have huge impact on the value of precision as precision is percentage of correct brackets out of total brackets of the tree parsed by parser.
;;; For instance , total brackets of parsed trees ( parsed by pasrse function)  = 100
;;;             Number of correct brackts out of these 100 = 80
;;;;          Precision = 80/100  = 0.80%
;;;;  If we make an efficient parser to include those unkown words and insteaf of returning empty tree it returns some closet tree
;;; In this case say Total brackets of parsed tree (including unknwn word trees) = 120
;;;;           Number of correct brackets out of these 120 =  100
;;;;; Precision = 100/120 = 0.83%
;;; Hence, depending on the correct brackets , precision may slightly decrease if we have accounted for those missing trees
;;; but in our current implementation , precision is not being affected by unkown words or empty trees .

;;; Impact on Recall
;;; Empty trees in this case are decreasing the overall value of Recall  as (recall (/ tcorrect tgold))
;;; for those missing trees , tcorrect is not being updated but tgold is everytime incremented either we find a tree or it's empty
;;; Becuase of these missing trees the overall F-score will also be lower than expected value because of decrease in recall.

 ;;; question 3 part d
 ;; The probablistic (one-best Viterbi) chart parser improves the parseval scores over the baseline version.
 ;; as baseline version will send the first tree that if finds for the input to parseval. This tree might not be the true representation for the input sentence.
 ;; As Viterbi computes the top-ranked tree based on rule-probabilities. These probabilties are calculated based on the occurances of rule in our gold standard
 ;; and hence higher the probability of rule --> higher is the correctness of our output. Without taking these probabilities into account , the baseline version
 ;; will have low parseval scores.  While viterbi improves the parseval score ( as it calculates the top ranked tree and sends to parse eval.


;;;; question 4
;;;; part a
;;; a new edge is added to agenda for every word of the input sentence.
;;; The condition that must be true to add new edge to  agenda , is that the input word must occur in our training data.
;;; if we could not find the lexemes for the input word then no edge will be added.(that means we have not seen that word in our training data)
;;; new edge has following properties
;;:                       from i :to (+ i 1)
;;;                      :category (lexeme-category lexeme)
;;;                      :daughters daughters
;;;                      :probability (lexeme-probability lexeme)
;;;
;;;       from : it tell us the starting point of edge ( index of the starting word in the sub-sequence/sub-sequence
;;;        to : index of next adjacent word in the input sentence
;;;        category : category associated with given input word
;;;        daughters : edge that is passive and composed of the word
;;;        probability : probability of occurance of word with category based on our training data
