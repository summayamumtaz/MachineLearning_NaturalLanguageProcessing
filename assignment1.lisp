...
;; third cond again checks the trailing punctuation mark for token. output for "goal." ---> "goal"
;; Note : the refined tokenize function is designed to remove only one punctuation mark at left side of token and maximum two puntuation marks at right side of token. Input "(goal.)" --> function output "goal"
(defun tokenize2 (s)
	   (loop
      ...
      for token = (string-downcase (subseq s start space))
	       ...
	       do ( cond ( (equal "" token) nil)
		        ( (not (alphanumericp (coerce (subseq token 0 1)  'character)))  (setf token  (string-left-trim (subseq token 0 1) token))))
	       do (cond ( (equal "" token) nil)
		       ( (not (alphanumericp (coerce (subseq token (- (length token) 1)) 'character ))) (setf token (string-right-trim (subseq token (- (length token) 1)) token))))
	       do (cond ( (equal "" token) nil)
		       ( (not (alphanumericp (coerce (subseq token (- (length token) 1)) 'character ))) (setf token (string-right-trim (subseq token (- (length token) 1)) token))))
...
;; count of unique word types is reduced based on refined approach. The most frequent token in both cases (original vs. Refined approach) is “the”. In original count for refine is 1781 and using refined approach it is 1781
...
(loop for k being the hash-keys in *hashtable1* using (hash-value v)
  maximize v
   return k)
;; Note : counts of refined function are  based on approach of removing one starting and maximum 2 ending punctuation marks
